...
EB_API EbErrorType svt_av1_enc_deinit(EbComponentType *svt_enc_component) {
    ...
    if (handle->scs_instance && handle->scs_instance->scs && handle->scs_instance->scs->static_config.fgs_table_file != NULL) {
        if (handle->scs_instance->scs->static_config.fgs_table) {
