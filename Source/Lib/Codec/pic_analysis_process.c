    ...
    uint32_t                 input_luma_origin_index)
{
    ...
    for (int32_t subb_y = 0; subb_y < ENERGY_NUM_BLOCKS_PER_DIM; subb_y++) {
        for (int32_t subb_x = 0; subb_x < ENERGY_NUM_BLOCKS_PER_DIM; subb_x++) {
            ...
            for (int32_t y = 0; y < ENERGY_BLOCK_SIZE; y++) {
                for (int32_t x = 0; x < ENERGY_BLOCK_SIZE; x++) {
                    printf("%6d ", abs(coeffs[y * ENERGY_BLOCK_SIZE + x]));
                }
                printf("\n");
            }*/
            ...
            for (int32_t coeff_idx = 1; coeff_idx < ENERGY_BLOCK_COEFF_COUNT; coeff_idx++) {
                if (lf_idx[coeff_idx] == 1) {
                    lf_energy += abs(coeffs[coeff_idx]);
                } else {
...
void *svt_aom_picture_analysis_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        if (!pcs->is_overlay) {
            ...
            // Gathering statistics of input picture, including Variance Calculation, Histogram Bins, and LF/HF Energy
            {
