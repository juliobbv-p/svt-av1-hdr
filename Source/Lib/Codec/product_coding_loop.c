...
static void md_stage_0(PictureControlSet *pcs, ModeDecisionContext *ctx,
    ...
    for (uint8_t itr = 0; itr < tot_itr; itr++) {
        for (uint32_t cand_idx = 0; cand_idx < fast_cand_count; cand_idx++) {
            ...
            if (pcs->scs->static_config.tune == 5 && pcs->ppcs->slice_type == B_SLICE) { // Only somewhat reverse the uni psy bias on B-Frames, prevents sharp / squiggling(lack of a better term?) artifacting
