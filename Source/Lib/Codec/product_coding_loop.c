...
static void md_encode_block(PictureControlSet *pcs, ModeDecisionContext *ctx, uint32_t sb_addr,
    ...
    // 3rd Full-Loop
    ctx->md_stage        = MD_STAGE_3;
    ctx->tune_ssim_level = (pcs->scs->static_config.tune == 2 && ctx->pd_pass == PD_PASS_1 && !pcs->scs->static_config.alt_ssim_tuning) ? SSIM_LVL_3 : 
    ((pcs->scs->static_config.tune == 4 || pcs->scs->static_config.alt_ssim_tuning) && ctx->pd_pass == PD_PASS_1) ? SSIM_LVL_1 : SSIM_LVL_0;
