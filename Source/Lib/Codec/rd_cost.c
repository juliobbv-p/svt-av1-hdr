...
uint64_t svt_aom_intra_fast_cost(PictureControlSet *pcs, struct ModeDecisionContext *ctx,
    ...
    if (svt_aom_allow_intrabc(&pcs->ppcs->frm_hdr, pcs->ppcs->slice_type) && cand->use_intrabc) {
        ...
        cand_bf->fast_luma_rate   = rate * (pcs->scs->static_config.tune == 5 ? 2 : 1);
        cand_bf->fast_chroma_rate = 0;
        uint64_t luma_sad         = luma_distortion * (pcs->scs->static_config.tune == 5 ? 2 : 1);
        uint64_t chromasad_       = chroma_distortion * (pcs->scs->static_config.tune == 5 ? 4 : 1);
        ...
        //Temporarily changed tune 3 to 5
        cand_bf->fast_luma_rate   = luma_rate * (pcs->scs->static_config.tune == 5 ? 2 : 1);
        cand_bf->fast_chroma_rate = chroma_rate * (pcs->scs->static_config.tune == 5 ? 4 : 1);
        luma_sad                  = luma_distortion * (pcs->scs->static_config.tune == 5 ? 2 : 1);
        chromasad_                = chroma_distortion * (pcs->scs->static_config.tune == 5 ? 4 : 1);
...
uint64_t svt_aom_inter_fast_cost(PictureControlSet *pcs, struct ModeDecisionContext *ctx,
    ...
    // Keep the Fast Luma and Chroma rate for future use
    //Temporarily change tune 3 to 5
    cand_bf->fast_luma_rate   = luma_rate * (pcs->scs->static_config.tune == 5 ? 2 : 1);
    cand_bf->fast_chroma_rate = chroma_rate * (pcs->scs->static_config.tune == 5 ? 4 : 1);
    luma_sad                  = luma_distortion * (pcs->scs->static_config.tune == 5 ? 2 : 1);
    chromasad_                = chroma_distortion * (pcs->scs->static_config.tune == 5 ? 4 : 1);
